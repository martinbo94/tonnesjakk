...
| 3v3 | 30.05B valid white-to-move states → **7.5 GB packed** (2-bit WDL, no DTW) | **1 h 44 min** (16 passes, 12 threads, ~10 GB RSS) | **side to move wins 62.45 %, loses 37.04 %, draws 0.518 %**. One-ply consistency 0/300 violations. Took four attempts: (1) macOS update reboot; (2) machine froze — 14 GB array + full 11.5 GB 3v2 table resident; (3) still swapped with 2-bit `.wdl` lower phases — everyday apps hold 25–30 GB logical; (4) valid-only indexing (7.5 GB) *and* the real culprit: the per-pass `(idx, value)` result buffer — pass 1 decides 15.65B states = >100 GB of buffer, swap hit 48 GB. Now assignments are written in place with atomic `fetch_or` (Gauss–Seidel; values only go unknown→decided so mid-pass reads are always true facts). Passes: 15.65B, 7.22B, 3.50B, 2.02B, 0.98B, … → 0 at pass 16. |
| 4v2 / 2v4 | 44.84B valid states (both sides to move) → **11.2 GB packed** | **4 h 04 min** (22 passes, 12 threads, ~15 GB RSS, lowmem) | side with 2 left wins **91.5 %**, side with 4 wins 5.6 %, **draws 2.9 %** (most drawish phase). 2v4 answered via the colour mirror. One-ply consistency 0/300 (both orientations). **Every position with ≤ 6 barrels remaining is now solved** (≤ 7 came later, with 4v3). |
| 4v3 / 3v4 | 366.7B valid → 91.7 GB packed | **SOLVED 2026-09-01** on the GCP spot VM: 24 passes, ~11.9 h (pass 1 alone 3.7 h; target-cpu=native rebuild bought ~nothing — the solver is memory-bound on the 92 GB array) | **side with 3 left wins 73.46 %, side with 4 wins 22.55 %, draws 3.99 %** (most drawish phase yet) |
| 4v4 | **1.085T valid → 271.2 GB packed** (white-to-move only) | ~400 GB RSS, 12–24 h on an n2-highmem-128 **spot ≈ $25–35 all-in** (zstd 16–44× on our tables → egress ~$3) | after this, the game is SOLVED: `value(initial position)` |
